...
//        JilterLog.edebug(">SMFIC command is '" + ((char) this.packet.getCommand()) + "', Raw packet data:" + Util.newline() + Util.hexDumpLong(this.packet.getData()));
	...
	throws IOException {
		...
		JilterLog.edebug("Recipient is \"" + argv[0] + "\"");
	...
	throws IOException {
		...
		argv = JilterServerPacketUtil.getZeroTerminatedStringArray(dataBuffer);
		JilterLog.edebug("Sender is \"" + argv[0] + "\"");
	...
	throws IOException {
		...
		JilterLog.edebug("Client identifier parsed as \"" + helohost + "\"");
	...
	private void processMacroPacket(WritableByteChannel writeChannel) {
		...
		for (int counter = 0; counter < propertiesStrings.length; counter += 2) {
			JilterLog.edebug("Setting property " + propertiesStrings[counter] + " = " + propertiesStrings[counter + 1]);
	...
	throws IOException {
		...
		if (dataBuffer.get() == JilterConstants.SMFIA_INET) {
...

			{
				...
				JilterLog.edebug("Parsed IP address is " + stringAddress);
